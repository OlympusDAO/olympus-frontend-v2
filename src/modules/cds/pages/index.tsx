...
export const CDPage = () => {
  ...
  const isDepositValid = React.useMemo(() => {
    ...
    if (!userAddress)
      return { valid: false, reason: "Connect wallet to deposit" };
  ...
  const displayTokenName =
    tokenName || `Receipt-${getTermSuffix(selectedTerm)}`;
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-secondary-t mt-1 flex items-center gap-2">
                <CircularProgress
                  value={(() => {
                    const tickSize =
                      currentTickSize ?? auctionParameters?.tickSize;
                    if (!tickData?.capacity || !tickSize) return 0;
                    return Math.min(
                      100,
                      (Number(tickData.capacity) / Number(tickSize)) * 100
                    );
                  })()}
                  size={16}
                  strokeWidth={3}
        ...
        {orderType === "market" ? (
                    ...
                    <div className="p-3 space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Switch
                            checked={wrapPosition}
                            onCheckedChange={setWrapPosition}
                          />
                        </div>
                      </div>
                    </div>
          ...
          <div className="bg-surface-a3 rounded-3xl p-4 space-y-3 border-a3-b border">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between border-b border-a5-b pb-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-medium">
                    {formatReceiptTokenAmount(depositAmount || "0")}{" "}
                    {displayTokenName}
                  </span>
                </div>
              </div>
              ...
              <div className="flex justify-between border-b border-a5-b pb-2">
                <span className="text-secondary-t font-light">
                  Conversion Expiry
                </span>
                <span className="font-medium">
                  {selectedTermMonths > 0
                    ? getConversionExpiryDate(selectedTermMonths)
                    : "--"}
                </span>
              </div>
            </div>
