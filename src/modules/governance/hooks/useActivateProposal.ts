...
import { trackActivateProposal, trackTransactionFailed } from "@/lib/analytics";
...
export function useActivateProposal() {
  ...
  useEffect(() => {
    if (isConfirmed && proposalIdRef.current != null) {
      trackActivateProposal({ proposalId: String(proposalIdRef.current), txHash: hash });
  ...
  useEffect(() => {
    if (error) {
      ...
      trackTransactionFailed("dao", "activate", { reason });
    }
  }, [error]);
